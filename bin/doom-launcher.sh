...
# - --last support (replay last successful launch)
#
...
LAST_RUN_FILE="$HOME/.doom-last-run"
...
save_last_run() {
  ...
  {
    print -r -- '#!/bin/zsh'
    print -rn -- 'LAST_DOOM_CMD=('
    printf '%q ' "${cmd[@]}"
    print -r -- ')'
  } >| "$LAST_RUN_FILE"
  chmod +x "$LAST_RUN_FILE"
}

run_last() {
  [[ -f "$LAST_RUN_FILE" ]] || die "No previous DOOM launch command saved."
  source "$LAST_RUN_FILE"
  [[ ${#LAST_DOOM_CMD[@]} -gt 0 ]] || die "Saved command file is invalid."
  print -r -- "Replaying last launch:"
  print -r -- "${(j: :)LAST_DOOM_CMD}"
  exec "${LAST_DOOM_CMD[@]}"
}
...
if [[ "$MODE_CHOICE" == "2" ]]; then
  ...
  case "$PRESET_CHOICE" in
    1)
      ...
      print -r -- ""
      print -r -- "Launching preset: Aliens TC 2017"
      print -r -- "  CONFIG: $PROFILE_CFG"
      print -r -- "  IWAD  : $ALIENS_IWAD"
      print -r -- "  DEH   : $ALIENS_DEH"
      print -r -- "  FILES : ${(j: :)ALIENS_LOAD}"
    ...
    2)
      ...
      print -r -- ""
      print -r -- "Launching preset: SIGIL"
      CMD=(
        "$ENGINE"
        -config "$PROFILE_CFG"
        -iwad "$WAD_DIR/doom.wad"
        -file "$MOD_DIR/sigil.wad"
        "${PROFILE_SETS[@]}"
      )
      launch_cmd "${CMD[@]}"
      ;;
    ...
    3)
      ...
      print -r -- ""
      print -r -- "Launching preset: SIGIL II"
      CMD=(
        "$ENGINE"
        -config "$PROFILE_CFG"
        -iwad "$WAD_DIR/doom.wad"
        -file "$MOD_DIR/sigil2.wad"
        "${PROFILE_SETS[@]}"
      )
      launch_cmd "${CMD[@]}"
      ;;
    ...
    4)
      ...
      print -r -- ""
      print -r -- "Launching preset: Master Levels"
      CMD=(
        "$ENGINE"
        -config "$PROFILE_CFG"
        -iwad "$WAD_DIR/doom2.wad"
        -file "$MOD_DIR/masterlevels.wad"
        "${PROFILE_SETS[@]}"
      )
      launch_cmd "${CMD[@]}"
      ;;
    ...
    5)
      ...
      print -r -- ""
      print -r -- "Launching preset: No Rest for the Living"
      CMD=(
        "$ENGINE"
        -config "$PROFILE_CFG"
        -iwad "$WAD_DIR/doom2.wad"
        -file "$MOD_DIR/nerve.wad"
        "${PROFILE_SETS[@]}"
      )
      launch_cmd "${CMD[@]}"
      ;;
...
(( ${#VALID_IWADS[@]} > 0 )) || die "No IWADs found in $WAD_DIR"

IWAD_CHOICE="$(read_choice_1key "Enter number: " "${#VALID_IWADS[@]}")" || die "Invalid IWAD selection."
SELECTED_WAD="${VALID_IWADS[$IWAD_CHOICE]}"

print -r -- ""
print -r -- "IWAD: $SELECTED_WAD"
print -r -- ""

print -r -- "Available mods (Aliens TC components hidden; use Presets → Aliens TC):"
if (( ${#MODS[@]} == 0 )); then
  print -r -- "  (none found)"
else
  j=1
...
print -r -- ""
print -r -- "Enter mod numbers separated by spaces to load (or press Enter to skip):"
read "MOD_SELECTION?Mods: "

SELECTED_MODS=()
for num in ${(s: :)MOD_SELECTION}; do
  [[ "$num" == <-> ]] || continue
