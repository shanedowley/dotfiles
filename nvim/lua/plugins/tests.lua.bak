    ...
    config = function()
        ...
        adapters = {
          gtest.setup({
            ...
            filter_dir = function(name, rel_path)
              ...
              return not (
                rel_path:match("^build/_deps") or -- CMake fetched deps (googletest, etc.)
                rel_path:match("^_deps")       or -- any _deps at repo root
                rel_path:match("^%.git")       or -- git internals
                false
                -- If you want to exclude *everything* under build/, add:
                -- or rel_path:match("^build/")
              )
            end,
          }),
        },
      ...
      local function relink_gtest_exec()
        ...
        for _, dir in ipairs(candidates) do
          ...
          if #found > 0 then exe = found[1]; break end
        end
        if not exe then
      ...
      map("<leader>tn", function() run_and_show() end,                          "Test: Run nearest")
      map("<leader>tf", function() run_and_show(vim.fn.expand("%")) end,        "Test: Run file")
      ...
      map("<leader>tR", function() neotest.run.run_last() end,                  "Test: Run last")
      ...
      vim.notify("[tests.lua] neotest + gtest configured", vim.log.levels.INFO)
    end,
  },
}
